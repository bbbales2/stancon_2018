...
It's the mechanical properties of these materials we want to understand. Here's a picture of a typical RUS cradle with a little sample resting in it.
...
3. Do higher level regressions
4. ...

Sometimes it's fun to send a model through the optimizer just to see what sorts of stuff happens.
...
1. Not all chains converge to a reasonable solution (some get stuck in places of parameter space far away from the data). We just discard these. It's not terribly satisfying, but there's not much else to do.
...
You'll notice the lack of $\hat{R}$ and $n_{eff}$ in the orientation table. Because there are numerous symmetries in the crystal and the sample, there are multiple symmetrically equivalent correct answers. I honestly don't know how to go about computing an $\hat{R}$ and $n_{eff}$ in this case. We usually just eyeball the convergence here.

It's hard to tell with the summary statistics, but the measured misorientation is kinda outside the posterior here. Its easier to see with a plot of the histogram of minimum angle of rotation between the measured and computed misorientations.

From this plot we can see that our computed and measured misorientations are within about a degree of each other. From an engineering standpoint, this is pretty close. From a statistics perspective, however, the measurement is outside the posterior, and these sorts of X-ray measurements can be done to within closer than a degree.
...
Looking back on the project, the big mistake we made at the beginning was assuming that because we have a simple mechanical problem (linear elasticity is quite easy) that it should also be easy to solve an inference problem attached to this. This turned out to be wrong, naturally.

And with all the gotchas and caveats listed above, it's not fair to say we totally solved our inference problem. I don't truly trust out confidence intervals, and I'm not sure I ever will.

I'm not convinced the product of this all are the uncertainties. What Bayesian inference gave us (and in particular, Stan/NUTS/HMC, whatever) that we did not have before is a way to very aggressively interrogate our models.

Even now, when we have what we consider a functional model, it is very clear what our biggest weaknesses are (the lack of diagnostics on orientations and the fact that sometimes chains don't converge to anything are the biggest). It isn't so easy to get this information, but it comes for free with Stan.

And it is for this reason that we strongly recommend that anyone who takes their science seriously should be figuring out ways to get their analysis into Stan. If something other piece of software comes along, great, but until then, you can find me browsing the Stan Forums (or at bbbales2@gmail.com).
