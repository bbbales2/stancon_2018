...
author: Ben Bales (UC, Santa Barbara)
...
The problem we worked on was the inference bit of Resonance Ultrasound Spectroscopy (or more shortly, RUS). RUS is the process of extracting elastic constants of materials by measuring the resonance modes of a sample of that material. A sample's shape and elastic properties (how difficult an object is to stretch and deform) determine its resonance modes. For the most part, we know how things are shaped, and we can measure where they resonate. The thing we want to back out of this are the material elastic properties (a couple standard references on this are [@visscher1991c] and [@migliori1993c]).

The driving application here are gas turbines (see: jet engines, and land based gas power generation). The bits of the gas turbine that need to get hot (the blades and the rotor especially) are made of special, high temperature resistant metals called superalloys. It's the elastic constants of superalloys that we want to know.
...
print(timing_base)
```
Not too shabby! The posterior contains the answer we were looking for ($k = 1.7$), and the $n_{eff}$ and \hat{R} values look good.
...
Woohoo! Our fit works ($k = 1.7$, and it runs about ten times faster than the original model). This is how you hook up custom external libraries with Stan models. Your mileage may vary, of course (especially if you have to link in other libraries), and it's best to test that the functions (as well as their gradients) are working outside of Stan before you try to incorporate them into a complicated model.
...
In modern superalloys, we're almost always interested in single crystal materials. This means that instead of a one parameter elasticity model, we work with somewhere between two and nine constants (depending on the symmetry of the crystal).
...
3. Computing eigenvalues is expensive (usually matrices are 1000x1000+ in these calculations). Making the calculations high enough resolution to avoid biasing the inference is difficult
4. The posteriors are frighteningly tight (we're a little bit off in no-mans land here -- nothing to compare against)
5. Orientations are difficult to work with
...
As far as the posterior predictive intervals go, things work out
```{r, echo=FALSE}
...
These sorts of X-ray measurements can be much more precisely than to within a degree. So it is pretty bad we're only within a few degrees of the answer. We're not that confident in the X-ray measurement we did though.
...
Looking back on the project, the big mistake we made at the beginning was assuming that because we have a simple mechanical problem (linear elasticity is quite easy) that it should also be easy to solve an inference problem attached to this. Hopefully the reader is convinced this is wrong as well.

And again, with all the problems above, we haven't really solved our inference problem yet. I don't truly trust out confidence intervals, which sounds pretty defeatist from a statistics perspective.

But I'm not convinced anymore that the confidence intervals are all we might hope to get from Bayesian modeling. We are much closer than ever before to understanding our problem, and Stan is indispensable as a tool for interrogating our models, data, and assumptions.

Even now, when we have what we consider a functional model, it is very clear what our biggest weaknesses are and how we might address these concerns.

And it is for this reason that we strongly recommend that anyone who takes their science seriously should be figuring out ways to get their analysis into Stan.
